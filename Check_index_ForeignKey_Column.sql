SELECT CASE
           WHEN ind.index_name IS NOT NULL THEN CASE
                WHEN ind.index_type IN ('BITMAP') THEN '** Bitmp idx **'
                ELSE 'indexed' END
           ELSE
               '** Check idx **' END checker
     , ind.index_type
     , cons.owner
         ...
         LEFT OUTER JOIN (SELECT table_owner,
                          ...
                          FROM (SELECT ic.table_owner,
                                ...
                                FROM dba_ind_columns ic,
                                     dba_indexes i
                                WHERE ic.table_owner = UPPER(':val_schema')
...
-- Check line 27 , 47 instead schema_name
